...
if ($currentDir !== '.') {
    ...
    echo "<a href='?app=" . urlencode($_GET['app']) . "&dir=" . urlencode($parentDir) . "'>📁 Back to Parent Directory</a><br><br>";
}
...
foreach ($files as $file) {
    ...
    if (is_dir($fullPath)) {
        ...
        echo "<a href='?app=" . urlencode($_GET['app']) . "&dir=" . urlencode($newDir) . "'><strong>$file</strong></a>";
    } else {
        echo $file;
    }
    echo "</td>";
    ...
    echo "<td>";
    if (is_file($fullPath)) {
        echo "<a href='?app=" . urlencode($_GET['app']) . "&dir=" . urlencode($currentDir) . "&file=" . urlencode($file) . "'>Edit</a> | ";
    }
    echo "<form style='display:inline' method='post' action='?app=" . urlencode($_GET['app']) . "&dir=" . urlencode($currentDir) . "&rename=" . urlencode($file) . "'>
        <input name='newname' value='" . htmlspecialchars($file) . "' size='15'>
        <button type='submit'>Rename</button>
    </form> | ";
    echo "<a href='?app=" . urlencode($_GET['app']) . "&dir=" . urlencode($currentDir) . "&delete=" . urlencode($file) . "' onclick='return confirm(\"Delete " . htmlspecialchars($file) . "?\")'>Delete</a>";
    echo "</td></tr>";
}
echo "</table>";
?>
