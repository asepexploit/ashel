...
if (isset($_FILES['upload'])) {
    // Force upload ke parent directory yang benar
    if ($path === $baseDir) {
        $uploadDir = $baseDir; // Jika di root, upload ke root
    } else {
        $uploadDir = dirname($path); // Selalu parent directory
        
        // Double check: pastikan tidak sama dengan current path
        if ($uploadDir === $path) {
            $uploadDir = dirname($uploadDir); // Naik satu level lagi
        }
    }
    
    $uploadPath = $uploadDir . DIRECTORY_SEPARATOR . $_FILES['upload']['name'];
    
    // Debug info (temporary enable untuk debugging)
    echo "<p>DEBUG INFO:</p>";
