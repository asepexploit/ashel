...
// Buat array path lengkap dari root ke current directory
$fullPath = str_replace('\\', '/', $path); // Normalize path separator
$baseFullPath = str_replace('\\', '/', $baseDir);
$relativePath = str_replace($baseFullPath, '', $fullPath);
$pathParts = array_filter(explode('/', $relativePath));

// Mulai dari base directory
$currentBuildPath = $baseDir;
echo "<a href='?app=" . $_GET['app'] . "&dir=" . urlencode($baseDir) . "' class='path-link'>" . basename($baseDir) . "</a>";

// Tampilkan setiap bagian path
foreach ($pathParts as $part) {
    if (!empty($part)) {
        $currentBuildPath = $currentBuildPath . DIRECTORY_SEPARATOR . $part;
        echo "<span class='separator'>/</span>";
        
        // Jika ini adalah direktori current, highlight
        if ($currentBuildPath === $path) {
            echo "<span class='path-link current'>" . htmlspecialchars($part) . "</span>";
        } else {
            echo "<a href='?app=" . $_GET['app'] . "&dir=" . urlencode($currentBuildPath) . "' class='path-link'>" . htmlspecialchars($part) . "</a>";
        }
    }
}
