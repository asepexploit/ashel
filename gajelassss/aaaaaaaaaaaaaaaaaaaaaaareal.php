...
$parts = explode(DIRECTORY_SEPARATOR, trim(str_replace($baseDir, '', $path), DIRECTORY_SEPARATOR));
$breadcrumb = "<a href='?app=" . $_GET['app'] . "&dir=" . urlencode($baseDir) . "'>$baseDir</a>";
$curPath = $baseDir;

foreach ($parts as $part) {
    if ($part === '') continue;
    $curPath .= DIRECTORY_SEPARATOR . $part;
    $breadcrumb .= " / <a href='?app=" . $_GET['app'] . "&dir=" . urlencode($curPath) . "'>$part</a>";
}

echo "<p><b>Current Directory:</b> $breadcrumb</p>";
