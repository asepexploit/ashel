...
function list_archives($dir) {
    $items = [];
    foreach (glob($dir . DIRECTORY_SEPARATOR . '*.zip') as $f) $items[] = $f;
    foreach (glob($dir . DIRECTORY_SEPARATOR . '*.rar') as $f) $items[] = $f;
    sort($items);
    return $items;
}
...
if ($TARGET) {
    ...
    if (empty($archives)) {
        out("Tidak ada file .zip atau .rar di direktori ini.");
        @unlink(__FILE__);
        exit;
    }
